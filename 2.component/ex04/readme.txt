ex02: Component - State

01. 기본개념
    - 컴포넌트 내부의 상태를 나타내는 쓰기 가능한 데이터
    - 컴포넌트 this.state 안에 여러 데이터(상태)를 가질 수 있다.
    - "상태가 변경되면 컴포넌트의 반응형(Reactive) 렌더링 트리거가 되고 컴포넌트와 자식 컴포넌트가 다시 렌더링 된다.
    - 클래스 컴포넌트에서는 constructor 안에서 초기화 된다.
    - 클래스 컴포넌트에서는 반드시 상태 변경을 위해서 setState 함수를 사용한다.
    - 함수 컴포넌트에서는 useState 훅(hook) 함수를 사용해서 초기화 한다.
    - 함수 컴포넌트에서는 상태 변경을 위해 useState 훅함수에서 반환된 두번째 요소를 함수로 사용한다.
